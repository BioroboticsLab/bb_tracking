...
def get_metrics_for_track(track, gt_track, track_false_positives):
    def series_for_track(t, is_false_positive):
        track_frames = []
        track_detection_keys = {d.timestamp: d.detection_index for i, d in enumerate(t.detections) if not is_false_positive(i, d)}
        for key in t.timestamps:
            det_index = track_detection_keys[key] if key in track_detection_keys else np.nan
            track_frames.append((key, float(det_index)))
        track_frames = pandas.DataFrame(track_frames, columns=["datetime", "idx_"])
    ...
    df["matching_detection"] = df.idx_detection == df.idx_gt
...
def calculate_metrics_for_tracking(track_generator, ground_truth_track_generator):

    gt_track_id_to_track = dict()
    gt_detection_to_track_id = dict()

    track_counter = 0
    for gt_track in ground_truth_track_generator:
        track_counter += 1
        ...
        for detection in gt_track.detections:
            ...
            gt_detection_to_track_id[(detection.frame_id, detection.detection_index)] = gt_track.id

    for (fid, didx), track_id in gt_detection_to_track_id.items():
        ...
        det_idx = [d.detection_index for d in track.detections]
        assert didx in det_idx
    ...
    def get_all_gt_tracks_for_track(track):
        ...
        for d in track.detections:
            key = (d.frame_id, d.detection_index)
    ...
    for track in track_generator:
        false_positives = [(d.frame_id, d.detection_index) not in gt_detection_to_track_id for d in track.detections]
